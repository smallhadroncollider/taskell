...
test_lists =
    testGroup "Data.Taskell.Lists" [
      ...
      , testGroup "count" [
            testCase "list exists" (
                assertEqual
                    "Returns length of middle list"
                    3
                    (count 1 testLists)
            )

          , testCase "list does not exist" (
                assertEqual
                    "Returns 0"
                    0
                    (count 10 testLists)
      ...
      , testGroup "changeList" [
          ...
          , testCase "out of bounds list" (
                assertEqual
                    "Nothing"
                    Nothing
                    (changeList (5, 1) testLists 1)
            )

          , testCase "out of bounds task" (
                assertEqual
                    "Nothing"
                    Nothing
                    (changeList (1, 10) testLists 1)
            )
        ]
      ...
      , testGroup "shiftBy" [
          ...
          , testCase "out of bounds list" (
                assertEqual
                    "Nothing"
                    Nothing
                    (shiftBy 5 1 testLists)
            )

          , testCase "out of bounds shift" (
                assertEqual
                    "Returns updated lists"
                    (Just  (fromList [list2, list1, list3]))
      ...
      , testGroup "appendToLast" [
            testCase "previous list exists" (
                assertEqual
                    "Returns updated lists"
                    (fromList [
                        list1
            ...
            testCase "previous list doesn't exist" (
                assertEqual
